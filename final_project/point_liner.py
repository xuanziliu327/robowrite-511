...
a21=np.array([0.3725,0.275625,0.17])
...
end_point=np.array([0.0, -2.32, -1.38, -2.45, 1.57, 0.0])
...
while not glfw.window_should_close(window):
    ...
    while (data.time - time_prev < 1.0/10):
        ...
        #i
            ...
            X_ref = QuadBezierInterpolate(q5, q6, q7, data.time - 4.2*t_total/n, 0.8*t_total/n)
  ...
  ################################################################################################################  
            ...
            X_ref = QuadBezierInterpolate(q47, q48, a1, data.time - 32*t_total/n, 0.5*t_total/n)
...
#轩
            ...
            X_ref = QuadBezierInterpolate(a51, a52, b1, data.time - 67.5*t_total/n, 0.5*t_total/n)
...
#5
    ...
    #3
            ...
            X_ref = LinearInterpolate(b105,b105, data.time - 126*t_total/n, 0.5*t_total)
        elif (data.time <127* t_total/n):
            X_ref = LinearInterpolate(b105,end_point, data.time - 126*t_total/n, 0.5*t_total)
            
        else:    
            X_ref = LinearInterpolate(end_point,end_point, data.time - 127*t_total/n, simend-t_total)
